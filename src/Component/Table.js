            ...
            <table className='w-full flex flex-col gap-[10px]'>
                <thead className='w-full'>
                    <tr className='flex w-full justify-around bg-[#87CEEB] rounded-md text-sm sm:text-base '>
                        <th className='p-4 sm:w-[42%]  justify-start text-left pl-[1.7vw] font-bold text-black'>Coin</th>
                        <th className='p-4 sm:w-[20%]  justify-start font-bold text-black'>Price</th>
                        <th className='p-4 sm:w-[18%]  justify-start font-bold text-black'>24h Change</th>
                        <th className='p-4 sm:w-[20%]  justify-start font-bold text-black'>Market Cap</th>
                    </tr>
                </thead>
                <tbody className='w-full'>
                    {arr?.filter((coin)=>coin.name.toLowerCase().includes(searchquery) || coin.symbol.toLowerCase().includes(searchquery)).slice(page * 10 - 10, page * 10).map((coin) => (
                        <tr onClick={() => clicked(coin.id)} className='text-xs vs:text-base w-full flex  hover:bg-[#87CEEB] text-[#FAF0E6] hover:font-bold hover:text-black rounded-md cursor-pointer' key={coin.id}>
                            ...
                            <td className='flex items-center p-4 w-[30%] sm:w-[42%]  '>
                            ...
                            <td className='p-4 w-[30%] sm:w-[20%] text-center'><button>{coin.current_price.toLocaleString()}</button></td>
                            <td className={`${getclass(coin.price_change_percentage_24h)} p-4 w-[10%] sm:w-[18%] text-center`}>
                                <button>{coin.price_change_percentage_24h.toFixed(2)} %</button>
                            </td>
                            <td className='p-4 w-[30%] sm:w-[20%] text-center'><button>{(coin.market_cap / 1000000).toFixed(0).toLocaleString()} M</button></td>
